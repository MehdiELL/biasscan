...
### AI article-bias analyzer on GenLayer
...
**Deployed contract (Testnet Bradbury):** [`0x4AAf9694F26644CA384eF9a026A34fC323CD83A7`](https://explorer-bradbury.genlayer.com/address/0x4AAf9694F26644CA384eF9a026A34fC323CD83A7)
...
2. **`evaluate(id)`** - each validator fetches the source, classifies the content
   (one of: neutral, slanted, heavily-biased) and scores it 0-100, then must **agree** on the label and a similar
   score via the Equivalence Principle.
3. The **label, score, and reasoning** are written on chain.

## The Intelligent Contract

`contracts/biasscan.py` targets the GenVM Python runner (pinned by hash):

- **Integers only** - the bias score is `u8` (0-100); no floats.
...
genvm-lint check contracts/biasscan.py

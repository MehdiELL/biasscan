...
# BiasScan - AI article-bias analyzer on GenLayer.
# Submit a URL; GenLayer validators independently evaluate it with an LLM, agree on a label and score by consensus, and write the result on chain. AI article-bias analyzer.
...
SCORE_TOLERANCE = 25
CATEGORIES = ("neutral", "slanted", "heavily-biased")
DEFAULT_LABEL = "slanted"
...
class BiasScan(gl.Contract):
    ...
    def evaluate(self, item_id: int) -> None:
        ...
        def leader_fn():
            ...
            prompt = f"""You are a neutral media analyst. Judge the ideological bias of the ARTICLE content, independent of which side it favors.
...
Classify the CONTENT as exactly one of: neutral, slanted, heavily-biased.
Also give an integer bias score from 0 to 100, and a one-sentence reason.

Respond with ONLY JSON: {{"label": "<one of: neutral, slanted, heavily-biased>", "score": <integer 0-100>, "reasoning": "<one sentence>"}}"""
